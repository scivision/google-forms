...
def analyzeForms(xlsfn,mplots,pick):
    ...
    rechoice(data,pick[0],'choice one')
    rechoice(data,pick[1],'choice two')
    rechoice(data,pick[2],'choice three')

    return data

def rechoice(data,choice,choicenum):
...
if __name__ == '__main__':
    ...
    if ar.profile:
        ...
        cProfile.run('analyzeForms(ar.infile,ar.plot, pick)',profFN)
        goCprofile(profFN)
    else:
        data = analyzeForms(ar.infile, ar.plot, pick)
        #print(data)